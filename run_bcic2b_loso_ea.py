"""Run E2EG2G on BCIC IV-2b with LOSO evaluation and optional EA.

The leave-one-subject-out (LOSO) protocol uses all training/evaluation
sessions from the held-out subject for testing, and all sessions from the
remaining subjects for training/validation.  When ``--ea`` is enabled, the
Euclidean Alignment transform is estimated only from the training subjects in
each fold and then applied to train/validation/test data.
"""
...
def fit_ea_transform(x):
    cov = np.matmul(x, np.transpose(x, (0, 2, 1)))
    ref = cov.mean(axis=0)
    eigvals, eigvecs = np.linalg.eigh(ref)
    eigvals = np.clip(eigvals, 1e-8, None)
    return (eigvecs @ np.diag(eigvals ** -0.5) @ eigvecs.T).astype(np.float32)


def apply_ea(x, transform):
    aligned = np.einsum("nct,rc->nrt", x, transform)
    return np.real(aligned).astype(np.float32)
...
def run(args):
    ...
    run_name = (
        f"bcic2b_loso_{'ea' if args.ea else 'noea'}_"
        f"L{args.latent_nodes}_f{args.lng_f1}_seed{args.seed}_{time.strftime('%Y%m%d_%H%M%S')}"
    )
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for x, y in train_loader:
                x_aug, y_aug = segment_augment(
                    x_train, y_train, args.n_aug, args.batch_size, 2, args.n_seg, 3, args.input_time
                )
            ...
            if epoch % args.print_every == 0:
                print(
                    f"BCIC2b LOSO {'EA' if args.ea else 'no-EA'} "
                    f"subject={subject:02d} epoch={epoch} val_loss={val_loss:.4f}",
                    flush=True,
                )
...
def parse_args():
    ...
    parser.add_argument("--ea", type=str2bool, default=False)
